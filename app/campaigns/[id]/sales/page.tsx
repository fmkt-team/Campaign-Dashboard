          ...
          <div key={item.label} className="flex items-center gap-4">
            ...
            <div className="flex-1 flex flex-col gap-1">
              <div className="flex items-center gap-1">
                <span className="text-[10px] text-white/30 w-8">전년</span>
                ...
                <span className="text-[10px] text-white/40 w-20 text-right">{item.v2025.toLocaleString()}</span>
              </div>
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] text-white w-20 text-right font-bold">{item.v2026.toLocaleString()}</span>
              </div>
            </div>
            <div className={`text-xs font-bold w-14 text-right shrink-0 ${
              growth > 0 ? "text-green-400" : growth < 0 ? "text-red-400" : "text-white/30"
            }`}>
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-white">매출 상세 분석</h2>
        ...
        <div className="flex gap-2">
          <label className="cursor-pointer">
            <input
              ...
              className="hidden"
            />
            <span className="inline-flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-md border border-white/20 text-white/80 hover:bg-white/10 transition-colors">
              <UploadCloud className="w-3.5 h-3.5" /> 엑셀 업로드
            </span>
          </label>
          <div className="text-xs text-white/30 flex items-center gap-1.5 border border-white/10 rounded-md px-3 py-1.5">
            또는 <kbd className="bg-white/10 px-1.5 py-0.5 rounded text-white/60">Ctrl+V</kbd> 로 붙여넣기
          </div>
        </div>
      </div>
      ...
      {salesData.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          <GlassCard className="p-5">
            <p className="text-xs text-white/40 mb-1">전년 총 수주액</p>
            <p className="text-2xl font-bold font-mono text-white/60">
              {(total2025 / 1e4).toFixed(0)}만원
            </p>
          </GlassCard>
          <GlassCard className="p-5">
            <p className="text-xs text-white/40 mb-1">올해 총 수주액</p>
          ...
          <GlassCard className="p-5 relative overflow-hidden">
            ...
            <p className="text-xs text-white/40 mb-1">전년 대비 성장률</p>
      ...
      {/* 탭 전환 */}
      <div className="flex items-center gap-1 bg-[#111] w-max p-1 rounded-lg border border-white/10">
            ...
            <Table>
              <TableHeader className="bg-white/5">
                <TableRow className="border-white/10 hover:bg-transparent">
                  {tab === "product" ? (
                    <>
                      <TableHead className="text-white/60">제품명</TableHead>
                      <TableHead className="text-white/60 text-right">전년 수주액</TableHead>
                      <TableHead className="text-white/60 text-right">올해 수주액</TableHead>
                      <TableHead className="text-white/60 text-right">증감액</TableHead>
                      <TableHead className="text-white/60 text-right">성장률</TableHead>
                    </>
                  ) : (
                    <>
                      <TableHead className="text-white/60">주차</TableHead>
                      <TableHead className="text-white/60 text-right">전년 수주액</TableHead>
                      <TableHead className="text-white/60 text-right">올해 수주액</TableHead>
                      <TableHead className="text-white/60 text-right">증감액</TableHead>
                      <TableHead className="text-white/60 text-right">성장률</TableHead>
                ...
                {(tab === "product" ? byProduct : byWeek).map((row: any, i) => {
                  ...
                  return (
                    <TableRow key={i} className="border-white/10 hover:bg-white/5 text-sm">
                      <TableCell className="font-medium text-white">{label}</TableCell>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-[#111] border border-white/20 rounded-2xl p-6 w-[600px] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-bold">매출 데이터 감지됨 ({pastedData.length}건)</h3>
              <button onClick={() => setPastedData(null)} className="text-white/40 hover:text-white">
                <X className="w-4 h-4" />
              </button>
            </div>
            <p className="text-xs text-white/40 mb-3">
              예상 형식: <span className="text-white/60">[제품명] [주차] [전년수주액] [금년수주액]</span>
            </p>
            <div className="overflow-y-auto max-h-[40vh] space-y-1 mb-6 border border-white/10 p-2 rounded-lg">
              ...
              {pastedData.length > 8 && (
                <div className="text-center text-xs text-white/30 pt-2">+ {pastedData.length - 8}행 더 있음</div>
              )}
            </div>
